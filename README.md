...
- **Antigravity AI Agent (Gemini 3.6 Flash)**: System architecture design, code generation, test suite crafting, and documentation.
- **Live LLM Integration (Groq API / `llama-3.3-70b-versatile`)**: Real-time sentiment analysis, tag extraction, and urgent feedback classification. Also supports Google Gemini (`GEMINI_API_KEY`) and OpenRouter (`OPENROUTER_API_KEY`).
...
2. **Atomic Seat Hold (`POST /seats/hold`)**: 5-minute (300s) hold grace period. Uses SQLite `BEGIN IMMEDIATE` transactions to prevent concurrent double-booking.
3. **Idempotent Booking Confirmation (`POST /bookings`)**: Converts active holds into confirmed bookings. Repeat calls return existing booking details.
4. **Time-Window Refund Engine (`POST /bookings/{id}/cancel`)**:
   - `> 24 hours` to departure: **100% Full Refund**
   - `6h to 24h` to departure: **50% Half Refund**
...
6. **AI Feedback & Sentiment Engine (`POST /bookings/{id}/feedback`)**:
   ...
   - **LLM Sentiment Classification**: Categorizes feedback (`POSITIVE`, `NEUTRAL`, `NEGATIVE`, `STRONGLY_NEGATIVE`), generates concise summary, extracts category tags, and sets `urgent_followup: true` for urgent complaints.
...
Your `.env` file is pre-configured with your Groq API key:
```env
GROQ_API_KEY=gsk_4gqY...
HOLD_GRACE_PERIOD_SECONDS=300
DATABASE_URL=bus_booking.db
```
