...
def seed_database():
    """Seed sample trips, seats, and users for testing and demonstration."""
    init_db()
    ...
    with get_db() as conn:
        ...
        for t in trips_data:
            ...
            for seat_num in seat_labels:
                cursor.execute("""
                    INSERT OR IGNORE INTO seats (trip_id, seat_number, status, updated_at)
                    VALUES (?, ?, 'available', ?)
        ...
        users = [
            ...
            ("USER-999", "Risk User", "risk@example.com", "+15559999999", 2, 0)  # Has 2 prior late cancellations
        ]
    ...
    print("Database successfully initialized and seeded with trips, seats, and users!")
